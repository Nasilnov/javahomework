...
public class Main {
    ...
    public  static boolean isPcCell(int y, int x) {
        return field[y][x] == PC_DOT;
    }
    ...
    public static int[] checkPosition (char c) {
        ...
        for (int x = 0; x < fieldSizeX; x++) {
            ...
            for (int y = 0; y < fieldSizeY; y++) {
                ...
                if (won ==  fieldWon - 2 ) {
                    ...
                    for (int yShift = y; yShift < fieldSizeY - 1 ; yShift++ ) {
                        if (isValidCell(x, yShift + 1) && isPcCell(x, yShift + 1)) {break;}
                        if (isValidCell(x, yShift + 1) && isEmptyCell(x, yShift + 1) && beg  < (fieldSizeY - fieldWon + 2)) {
            ...
            for (int y = 0; y < fieldSizeY; y++) {
                ...
                if (won ==  fieldWon - 2 ) {
                    ...
                    int beg = y;
                    for (int yShift = y; yShift < fieldSizeY - 1; yShift++ ) {
                        if (isValidCell(yShift + 1, x) && isPcCell(yShift + 1, x)) {break;}
                        if (isValidCell(yShift + 1, x) && isEmptyCell(yShift + 1, x) && beg  < (fieldSizeY - fieldWon + 2)) {
        ...
        for (int xShift = 0; xShift <= (fieldSizeX - fieldWon); xShift++) {
            for (int yShift = 0; yShift <= (fieldSizeY - fieldWon); yShift++) {
                ...
                for (int x = xShift, y = yShift; x < fieldWon + xShift && y < fieldWon + yShift; x++, y++) {
                    ...
                    if (won == fieldWon - 2) {
                        ...
                        for (int yShiftIn = y, xShiftIn = x; yShiftIn < fieldSizeY - 1; yShiftIn++, xShiftIn++ ) {
                            if (isValidCell(xShiftIn + 1, yShiftIn + 1) && isPcCell(xShiftIn + 1, yShiftIn + 1)) {break;}
                            if (isValidCell(xShiftIn + 1, yShiftIn + 1) && isEmptyCell(xShiftIn + 1, yShiftIn + 1)
                                    && beg  < (fieldSizeY - fieldWon + 2) ) {
                ...
                won = 0;
                for (int x = fieldWon - 1 + xShift, y = yShift; x >= xShift; x--, y++) {
                    if (field[x][y] == c) {
                        won++;
                    }
                    else {
