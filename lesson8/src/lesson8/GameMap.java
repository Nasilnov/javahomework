...
//        System.out.println("----------------");
    ...
    private boolean turnHumanWinCell() {
        ...
        for (int i = 0; i < fieldSizeY; i++) {
            for (int j = 0; j < fieldSizeX; j++) {
                if (isEmptyCell(j, i)) {
                    field[i][j] = DOT_HUMAN;            // поставим крестик в каждую клетку по очереди
                    if (checkWinForTurnAi(DOT_HUMAN)) {            // если игрок победит
                        field[i][j] = DOT_AI;            // поставить на то место нолик
                        return true;
                    }
    ...
    public  boolean isHumanCell(int y, int x) {
    ...
    private boolean checkWinForTurnAi(int labelPlayer) {
        for (int i = 0; i < fieldSizeX; i++) {            // ползём по всему полю
            for (int j = 0; j < fieldSizeY; j++) {
                if (checkLine(i, j, 1, 0, winLength - 1, labelPlayer)) {
                    return true;    // проверим линию по х
                }
                if (checkLine(i, j, 1, 1, (winLength - 1), labelPlayer)) {
                    return true;    // проверим по диагонали х у
                }
                if (checkLine(i, j, 0, 1, winLength - 1, labelPlayer)) {
                    return true;    // проверим линию по у
                }
                if (checkLine(i, j, 1, -1, winLength - 1, labelPlayer)) {
                    return true;    // проверим по диагонали х -у
